  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const uploadFile = async (file: File | null, bucket: string, name: string) => {
        ...
        const ext = allowedTypes[contentType] ?? 'jpg';
      ...
      if (idFrontUrl instanceof Response) return idFrontUrl;
      const idBackUrl = await uploadFile(idBack, 'kyc-documents', 'id_back');
      if (idBackUrl instanceof Response) return idBackUrl;
      const selfieUrl = await uploadFile(selfie, 'kyc-documents', 'selfie');
      if (selfieUrl instanceof Response) return selfieUrl;

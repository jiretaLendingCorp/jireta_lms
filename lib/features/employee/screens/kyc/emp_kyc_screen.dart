// lib/features/employee/screens/kyc/emp_kyc_screen.dart
...
class EmpKycScreen extends ConsumerStatefulWidget {
  const EmpKycScreen({super.key});

  @override
  ConsumerState<EmpKycScreen> createState() => _EmpKycScreenState();
}

class _EmpKycScreenState extends ConsumerState<EmpKycScreen>
...
class _KycList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final kycAsync = ref.watch(empKycProvider(status));
          ...
          itemBuilder: (_, i) {
            ...
            return Card(
              child: Padding(
                ...
                child: Row(
                  children: [
                    Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: AppColors.accent.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.verified_user_outlined, size: 20, color: AppColors.accent)),
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      IconButton(
        ...
        onPressed: () async {
          ...
          final res = await ref.read(empRepositoryProvider).approveKyc(widget.kycId);
          setState(() => _loading = false);
          if (context.mounted) {
            context.showSnack(res.success ? 'KYC approved' : res.error!, isError: !res.success);
            if (res.success) { ref.invalidate(empKycProvider('pending')); ref.invalidate(empKycProvider('all')); }
          }
        },
        tooltip: 'Approve',
      ),
      IconButton(
        ...
        onPressed: () async {
          ...
          final ok = await showDialog<bool>(context: context, builder: (dialogCtx) => AlertDialog(
            ...
            actions: [TextButton(onPressed: () => Navigator.pop(dialogCtx, false), child: const Text('Cancel')), FilledButton(onPressed: () => Navigator.pop(dialogCtx, true), style: FilledButton.styleFrom(backgroundColor: AppColors.error), child: const Text('Reject'))],
          ));
          if (ok != true || ctrl.text.trim().isEmpty) return;
          setState(() => _loading = true);
          final res = await ref.read(empRepositoryProvider).rejectKyc(widget.kycId, ctrl.text.trim());
          setState(() => _loading = false);
          if (context.mounted) {
            context.showSnack(res.success ? 'KYC rejected' : res.error!, isError: !res.success);
            if (res.success) ref.invalidate(empKycProvider('all'));
          }
        },
        tooltip: 'Reject',
      ),

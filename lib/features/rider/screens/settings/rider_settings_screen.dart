...
class _RiderSettingsScreenState extends ConsumerState<RiderSettingsScreen> {
  ...
  void dispose() {
    ...
    _confirmCtrl.dispose(); _distanceCtrl.dispose();
    _fuelPriceCtrl.dispose(); _consumptionCtrl.dispose();
    super.dispose();
  }
  ...
  void _computeGas() {
    ...
    if (distance == null || price == null || consumption == null || consumption == 0) {
      context.showSnack('Enter valid values', isError: true); return;
    }
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: const SingleChildScrollView(child: Column(
        ...
        children: [
          ...
          Text('📞  +63 2 8888 0000'),
          SizedBox(height: 16),
          Text('Rider Guidelines', style: TextStyle(fontWeight: FontWeight.w700)),
          SizedBox(height: 6),
          Text('• Always confirm the lender\'s identity before collecting.\n'
    ...
    return SafeArea(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            child: Row(children: [
              GestureDetector(
                ...
                child: Stack(children: [
                  AppAvatar(imageUrl: user?.avatarUrl, name: user?.displayName ?? '',
                      size: 56, backgroundColor: accent),
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _GlassBadge(label: 'Rider', color: accent),
              ])),
          ...
          GlassCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _GlassHeader(icon: AppIcons.profile, title: 'Account Info'),
              const SizedBox(height: 12),
              _GlassInfoRow(label: 'Member Since', value: user?.createdAt.toDisplayDate ?? '—'),
              const SizedBox(height: 6),
              _GlassInfoRow(label: 'User ID', value: user?.id.substring(0, 8).toUpperCase() ?? '—'),
              const SizedBox(height: 6),
              _GlassInfoRow(
                  label: 'Status',
                  value: user?.isActive == true ? 'Active' : 'Inactive',
                  valueColor: user?.isActive == true ? AppColors.success : AppColors.error),
              if (user?.phone != null) ...[ const SizedBox(height: 6), _GlassInfoRow(label: 'Phone', value: user!.phone!) ],
              if (user?.address != null) ...[ const SizedBox(height: 6), _GlassInfoRow(label: 'Address', value: user!.address!) ],
            ]),
          ),
          const SizedBox(height: 12),
          ...
          _GlassCollapsibleSection(
            icon: AppIcons.profile, title: 'Edit Profile',
            ...
            child: Column(children: [
              ...
              AppTextField(label: 'Address', controller: _addressCtrl,
                  isGlass: true, maxLines: 2, maxLength: 200,
                  textCapitalization: TextCapitalization.sentences),
              const SizedBox(height: 16),
              AppButton(label: 'Save Changes', color: accent, textColor: Colors.black87,
          ...
          // ── Security ──────────────────────────────────────────────────────
          _GlassCollapsibleSection(
            icon: AppIcons.lock, title: 'Security',
            ...
            child: Column(children: [
              ...
              AppTextField(label: 'Confirm New Password', controller: _confirmCtrl,
                  isGlass: true, obscureText: true),
              const SizedBox(height: 16),
              AppButton(label: 'Change Password', color: accent, textColor: Colors.black87,
          ...
          GlassCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _GlassHeader(icon: AppIcons.truck, title: 'Gas Cost Estimator'),
              const SizedBox(height: 4),
              Text("Estimate fuel cost to reach a lender's address",
                  style: TextStyle(color: Colors.white.withValues(alpha: 0.55), fontSize: 12)),
              const SizedBox(height: 16),
              AppTextField(label: 'Distance to Lender (km)', controller: _distanceCtrl,
                  isGlass: true,
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  hint: 'e.g. 12.5'),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(child: AppTextField(label: 'Fuel Price (₱/L)', controller: _fuelPriceCtrl,
                    isGlass: true, keyboardType: const TextInputType.numberWithOptions(decimal: true))),
                const SizedBox(width: 12),
                Expanded(child: AppTextField(label: 'Fuel Economy (km/L)', controller: _consumptionCtrl,
                    isGlass: true, keyboardType: const TextInputType.numberWithOptions(decimal: true))),
              ]),
              const SizedBox(height: 16),
              AppButton(label: 'Calculate', color: accent, textColor: Colors.black87,
                  onPressed: _computeGas, width: double.infinity),
                  ...
                  child: Row(children: [
                    ...
                    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text('₱${_estimatedGas!.toStringAsFixed(2)}',
                          style: const TextStyle(color: accent, fontSize: 22, fontWeight: FontWeight.w800)),
                    ]),
                  ]),
                ),
              ],
          ...
          GlassCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _GlassHeader(icon: AppIcons.settings, title: 'Preferences'),
              const SizedBox(height: 8),
              _GlassSwitchTile(icon: AppIcons.notifications, label: 'Push Notifications',
                  subtitle: 'Receive assignment alerts', value: _notifEnabled,
                  onChanged: (v) => setState(() => _notifEnabled = v)),
              _GlassDivider(),
              _GlassSwitchTile(icon: AppIcons.mapPin, label: 'GPS / Location',
                  subtitle: 'Enable for navigation', value: _locationEnabled,
                  onChanged: (v) => setState(() => _locationEnabled = v)),
              _GlassDivider(),
              _GlassSwitchTile(icon: isDark ? AppIcons.sun : AppIcons.moon,
          ...
          GlassCard(
            ...
            child: Column(children: [
              _GlassTapTile(icon: AppIcons.headphones, label: 'Help & Support',
                  onTap: _showHelpDialog),             // FIX: now opens dialog
              _GlassDivider(),
              _GlassTapTile(icon: Icons.privacy_tip_outlined, label: 'Privacy Policy',
                  onTap: _showPrivacyDialog),          // FIX: now opens dialog
              _GlassDivider(),
              _GlassTapTile(icon: Icons.description_outlined, label: 'Terms & Conditions',
...
class _GlassCollapsibleSection extends StatelessWidget {
  ...
  final Widget child;
  const _GlassCollapsibleSection({required this.icon, required this.title,
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Container(padding: const EdgeInsets.all(7),
          decoration: BoxDecoration(color: AppColors.riderAccent.withValues(alpha: 0.15),
              borderRadius: BorderRadius.circular(8)),
...
class _GlassSwitchTile extends StatelessWidget {
  ...
  const _GlassSwitchTile({required this.icon, required this.label,
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Switch(value: value, onChanged: onChanged,
            activeTrackColor: AppColors.riderAccent, activeThumbColor: Colors.white),
      ]),
    );
  }
}
...
class _GlassTapTile extends StatelessWidget {
  ...
  const _GlassTapTile({required this.icon, required this.label,
...
class _GlassInfoRow extends StatelessWidget {
  ...
  const _GlassInfoRow({required this.label, required this.value, this.valueColor});
  @override
  Widget build(BuildContext context) {
